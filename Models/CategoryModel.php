...
class CategoryModel extends Manager
{
    ...
    public function fetchAll()
    {
        ...
        $sql = "SELECT * FROM `$this->categoryTableName` ORDER BY shop_category_weight DESC;";
        $returnArray = [];
        $req = $db->prepare($sql);
        if ($req->execute()) {
    ...
    public function setWeightList(array $catrgories_id): bool
    {
        $returnBool = true;
        $weightValue = count($catrgories_id);

        foreach ($catrgories_id as $categoryId) {
            $params = array(
                'categoryId' => (int)$categoryId,
                'weightValue' => $weightValue
            );

            var_dump($categoryId, $weightValue);

            $sql = "UPDATE `$this->categoryTableName` SET shop_category_weight=:weightValue WHERE shop_category_id=:categoryId";

            $db = Manager::dbConnect();
            $req = $db->prepare($sql);
            $res = $req->execute($params);
            $returnBool = $returnBool === true && $res;
            var_dump($res);
            $weightValue--;
        }
